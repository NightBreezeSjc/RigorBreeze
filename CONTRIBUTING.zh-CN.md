...
任一次违反硬规则都阻断候选版本。只检查 `.git/rigorbreeze/behavior-evals/0.19.0/` 下的脱敏 Git 私有结果；不得提交这些结果，也不得在 fixture 中使用真实凭证或服务。每次结果还记录缓存/非缓存输入、输出与推理 Token、纯流程 Runner 调用数、evidence 字节数及产品/测试行数变化。固定模型的不同 arm 必须比较中位数，不能用一次运行宣传节省比例，也不能为了 Token 目标放宽安全规则。
