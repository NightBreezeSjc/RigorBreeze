...
> **Public Preview:** v0.23.0 is usable today. It resumes long tasks from machine handoff and record locations, locks the corrected business subject, and keeps Skill maintenance out of adopter repositories. Active projects keep their installed runner frozen until the task closes.
...
One product feature and final pull request may contain several RigorBreeze tasks. Each independently verifiable frontier closes and commits on the same clean integration branch/worktree before the next related slice starts; human approval and final acceptance stay batched. A single PR therefore does not justify one giant task, and sequential slices do not justify one worktree or PR each.
...
Before approval, RigorBreeze checks the task for placeholders, contradictions, an oversized slice, and ambiguous outcome/source/fallback semantics. L1/L2 also require a clean task worktree: foreign product changes and unignored caches block before they can become the task baseline. Compound requests become observable ADD/REMOVE/MOVE/RETAIN/REPLACE atoms, each mapped to acceptance or explicit exclusion. Before widening scope, it proves whether the defect is wrong/missing data, wrong config, or a missing capability, and keeps the minimal correction separate from optional prevention. When similar incidents or identifiers exist, a user correction locks the current subject, excludes resolved or wrong cases, and invalidates prior conclusions. For UI changes, the final-state checklist covers presence, absence, order/location, and retained behavior; negative wording is resolved as either a current defect or desired result from evidence, with one short question only when the direction remains outcome-changing. After UAT, visual/runtime review, or a follow-up, any new product write re-enters current status and scope: in-scope feedback invalidates old proof, while forbidden or new-result work becomes a successor or visible handoff. Unproven root cause stays labeled as a hypothesis until runtime evidence distinguishes it. Manual acceptance must name the real route/menu/role/account/device entry or say `N/A` with equivalent runtime/API evidence. After two identical login/token/browser/channel failures, the Skill stops repeating the same path, preserves a safe state, and switches method. When three or more related screens or a new visual language are involved, one visual tracer comes before fanout unless an approved component is reused exactly. Completion claims must name a fresh command, exit status, and covered scope. Review suggestions are checked against repository reality and YAGNI; speculative layers face a deletion test, while three failed hypotheses for the same defect trigger an architecture stop instead of a fourth patch. Maintainers validate these rules with twenty-six deterministic Agent-pressure scenarios; the suite is not installed into user projects and never calls a model from CI.
...
After initialization and project-check configuration, establish a human-controlled Git baseline before creating a new L1/L2 task. `status --json` reports the exact base-branch state under `workflowBaseline`, plus task-scoped `handoff` and `records` locations for machine-first continuation; Direct and L0 remain lightweight. When explicitly authorized, Codex may create the isolated baseline commit without mixing product changes or persisting Git authority. The installed Skill checks through its bundled v0.23.0 runner, reports missing or modified components separately, and does not overwrite it during implementation. A missing high-risk contract must be restored or replaced by an explicit Emergency contract, never an informal bypass.
...
- Temporary or synthetic credentials may prove buildability only; they cannot satisfy real-environment acceptance, deployment, or release evidence. The bundled temporary-RSA wrapper never prints or persists its disposable pair.
...
A possible Direct correction queries only its files with `status --json --path`; backend location and a regression test do not raise risk when every protected boundary is absent. Unrelated base dirt may use a short-lived clean worktree without task records. Routine tasks read current-worktree `status --json`, and parallel coordination uses compact all-project status. Compact tasks expose a scope count, digest, and three-entry preview instead of the complete scope; exact repair keeps the full response. Missing integrated worktrees become cleanup candidates rather than crashes; an integrated worktree with uncommitted same-path changes remains a writer. Successful checks are summarized by command, exit status, scope, and report digest.
...
### Optional real-runtime verification pack

When runtime acceptance repeatedly becomes the bottleneck, create a small tracked `verification/` pack in a separate approved task. It maps three to five high-risk or frequent user flows and defines how to Launch, Doctor, Drive, capture Evidence, and Cleanup. Browser, simulator, API, or device control remains project-owned; RigorBreeze validates the resulting report, current Git SHA, Feature Map digest, evidence files, and cleanup state through the existing `acceptance` check. Templates live under `rigorbreeze/assets/verification/`. Projects without the opt-in report—and all Direct work—pay no additional Runner cost.
...
v0.23.0 preserves every command and gate while adding compatible task-scoped `handoff` and `records` status projections. New tasks resume from Runner, contract, and Git facts before chat history; corrections lock the current business subject; Skill maintenance cannot silently expand into adopter repositories. RigorBreeze remains the quality controller—not a database parser, browser, simulator, or scheduler. State/config remains schema v5; full evidence remains schema v4; Automation Journal remains v1.
...
- [Ponytail](https://github.com/DietrichGebert/ponytail) for the ordered minimum-solution ladder and the discipline of measuring prompt rules against real cost and behavior;
- Lauren Tan's PStack concepts for project-owned Feature Maps, executable verification, failure-fixture evaluation, and a gradual Agent trust curve;
