...
> **Public Preview:** v0.21.0 is usable today. It retains v0.20.1 verification and sequential-task ownership while separating old-schema preflight from post-migration assertions. Active projects keep their installed runner frozen until the task closes.
...
After initialization and project-check configuration, establish a human-controlled Git baseline before creating a new L1/L2 task. `status --json` reports the exact base-branch state under `workflowBaseline`; Direct and L0 remain lightweight. When explicitly authorized, Codex may create the isolated baseline commit without mixing product changes or persisting Git authority. The installed Skill checks through its bundled v0.21.0 runner, reports missing or modified components separately, and does not overwrite it during implementation. A missing high-risk contract must be restored or replaced by an explicit Emergency contract, never an informal bypass.
...
A possible Direct correction queries only its files with `status --json --path`; backend location and a regression test do not raise risk when every protected boundary is absent. Unrelated base dirt may use a short-lived clean worktree without task records. Routine tasks read current-worktree `status --json`, and parallel coordination uses compact all-project status. Missing integrated worktrees become cleanup candidates rather than crashes; an integrated worktree with uncommitted same-path changes remains a writer. Successful checks are summarized by command, exit status, scope, and report digest; full all-project status remains reserved for exact repair, cleanup, and evolution.
