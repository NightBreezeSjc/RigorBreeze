...
> **Public Preview:** v0.16.0 is usable today. It preserves the v0.15 reliability gates while reducing solution and workflow waste through an ordered reuse ladder, batched cross-repository interaction, bounded evidence, and maintainer-only Token telemetry. Active projects keep their installed runner frozen until the task closes.
...
After initialization and project-check configuration, establish a human-controlled Git baseline before creating a new L1/L2 task. `status --json` reports the exact base-branch state under `workflowBaseline`; Direct and L0 remain lightweight. When explicitly authorized, Codex may create the isolated baseline commit without mixing product changes or persisting Git authority. The installed Skill checks through its bundled v0.16.0 runner, reports missing or modified components separately, and does not overwrite it during implementation. A missing high-risk contract must be restored or replaced by an explicit Emergency contract, never an informal bypass.
...
v0.16.0 keeps every v0.15 consequence, TDD, acceptance, and delivery gate. It adds an ordered solution ladder, one combined human interaction for a cross-repository result while retaining repository-local proof, online evidence compaction, and Git-private Token telemetry for honest A/B evaluation. It does not install Ponytail or add modes, hooks, debt comments, public commands, dependencies, or cross-repository state. State/config remains schema v5; full evidence remains schema v4.
