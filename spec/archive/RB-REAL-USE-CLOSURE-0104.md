...
- Current behavior and evidence: In two application repositories, `status --all --json` aborts when a registered active contract is missing while `doctor --all --json` reports no issue; a client-app `workflow-bypass` candidate stayed buried in evidence; new L1/L2 work can be framed before installation/baseline drift is resolved; one production-release task continued with a manual card after workflow state failed; an intentional new-domain draft was later mistaken for disposable residue because its origin and waiting condition were not machine-visible.
