...
- 只读扫描器和证据生成器默认保持 L1；只有可能写入迁移、凭证、生产、基础设施或发布状态时才升级。跟踪精简清单和摘要，原始清单留在本地或有期限的 CI Artifact。
- 源码完整性和归属声明必须由独立归属预期证明；文件名、关键词、正则和语义启发式只能筛选候选，不能自证完整。

### 验证

- 确定性 Agent 行为合同从十一个扩展为十三个场景，新增业务任务/流程缺陷隔离和顺序项目证据经济性场景。
