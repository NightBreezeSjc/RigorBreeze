...
Before product-code writes, invoke `$rigorbreeze`, inspect bundled-runner status, and require an approved task plus a successful window claim. Read `rigorbreeze.toml`, `spec/index.md`, and the active contract.
...
Use `archive --outcome abandoned --reason <reason>` for a clean cancelled task; preserve its branch and worktree. Conditional L2 integrations map enabled/disabled/unavailable behavior in `Operational-Modes`, and L2 remote release requires an operation plan with one safe recovery entry.
