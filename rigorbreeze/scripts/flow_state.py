...
Local mode is advisory; CI, L2, merge, and release use enforced profiles. L0 closes after configured verification; L1/L2 require current full verification, applicable acceptance, review, and retrospective confirmation. Read-only scanner/evidence tooling remains L1 unless it writes migration, credentials, production, infrastructure, or release state. Verify review feedback against the requirement, actual use, compatibility constraints, tests, and YAGNI before implementation. Completeness/ownership claims require an oracle independent of filename, token, regex, or semantic heuristics. Immutable artifacts and release governance are required only when release is actually requested.
...
When a business task exposes a reusable workflow defect, preserve its contract, record the blocker/evolution candidate, and create a separate RigorBreeze Skill task; never widen the business scope or patch its private runner.
{AGENTS_END}"""
