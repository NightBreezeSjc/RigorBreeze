...
Complete an imperfect prompt before creating the contract. Recover **recoverable project facts** from authoritative requirements, current code, tests, Git, interfaces, data, permissions and runtime evidence. Before any product write, resolve and read every authoritative requirement, prototype, design, API, or runtime source linked by the task/index or discovered in the affected slice; code and tests cannot replace an applicable product source. Ask only for **outcome-changing intent** those sources cannot establish. State safe defaults and uncertainty explicitly; a persona such as “CTO” is a perspective, never evidence or approval. Trace only the affected vertical slice and its dependencies rather than rereading the whole repository.

Use **initiative shaping** before task creation only for a new product, new business domain, broad legacy migration, or other unshaped initiative that cannot yet name one stable user outcome. Do not create a delivery task while shaping. Recover facts first, then maintain a **decision frontier**: ask at most **three questions** per round, limited to currently answerable outcome-changing choices, and include a recommendation, reason, and result/scope impact for each; recompute the frontier after the answers and ask nothing when evidence already settles the result. Compare two or three viable approaches and maintain one compact, versioned brief covering the problem, target user and journey, desired outcome and measure, evidence versus assumptions, value, usability, feasibility, and viability risks, appetite, rabbit holes, no-gos, and the recommended first vertical slice. A prototype answers **one decision question**; record its reference, observation, and verdict in the brief, keep it disposable by default, and never treat it as production implementation or acceptance. Obtain user approval only after the frontier is empty and assumptions are explicit, then cite that brief in only the first task. Ordinary bounded work skips shaping; use the same bounded frontier only for genuinely branching L2 ambiguity. Reference code proves prior behavior, never current product intent.

Write the compact result into the existing Authoritative inputs: user outcome, current behavior and evidence, business and architecture path, invariants and source of truth (including freshness/fallback semantics), requirement/design/API version, and unresolved outcome-changing ambiguity. For a defect, explicitly classify it as wrong/missing data, config, or missing capability **before any product write**; an unspoken inference does not complete this check. Preserve a short exact user phrase or resolvable source, then translate every compound request into **observable atoms** (`ADD`, `REMOVE`, `MOVE`, `RETAIN`, or `REPLACE`) stated as final product behavior. Map each atom to an acceptance ID or an explicit out-of-scope reason; keep the minimal correction separate from optional prevention and do not create another context report or make the developer fill these fields.

## Frame one vertical slice

Risk follows consequence, not diff size or elapsed time. Choose the lane:

- `Direct`: one low-consequence result in one repository, no outcome ambiguity or active competing writer, and no API/data shape, auth, permission, payment, lock, migration, dependency, production configuration, or release impact. Choose this lane before task creation; read status, reproduce or confirm, make the smallest edit, run one direct targeted check, and report command/exit/scope. Create no task, worktree, Spec, or evidence, and do not promote merely because configured profiles are absent or incomplete; promote only when the product scope crosses a boundary.
...
Before approval, perform one **semantic self-review** for placeholders, contradictions, oversized scope, ambiguous outcome/source-of-truth/freshness/fallback meaning, and any root cause as a hypothesis rather than a fact until discriminating runtime evidence exists. For negated wording such as “not shown”, “missing”, or “not changed”, distinguish a **current defect** from the **desired result** using project evidence; if evidence cannot decide, ask one short outcome question. For UI work, cover what must exist, must be absent, its order/location, and retained behavior; when three or more related screens or a new visual language are involved, identify one representative screen as the tracer unless an approved component is reused exactly. Show one compact **final-state checklist** before approval, repair recoverable facts directly, and ask only when ambiguity changes the result. Approve only after every atom is covered and scope, design, acceptance, plan, and test shape are stable. Its digest becomes the contract; changes invalidate downstream evidence. Do not reapprove over production implementation changes. Restore the approved contract and finish, or revert production changes before amending and reapproving the same outcome. Create a dependent task for a new user outcome or acceptance condition.
...
For a multi-screen visual tracer identified during approval, the approved multi-screen request authorizes implementation of exactly that one representative screen. Implement it, run fresh visual/behavior verification, and only then stop for human visual approval before touching the remaining screens. Do not ask for visual approval before producing the tracer; a plan, contract, branch, or status result is not a visual tracer.

Before writing a custom mechanism or adding a dependency, inspect the **standard library, framework, and current dependencies**. Prefer the smallest maintained capability that satisfies the contract. A new dependency, abstraction, helper, or configuration layer needs **current acceptance or a durable invariant**; preserve the smallest working vertical path first, then extract a boundary only for independent change or safety. Apply a **deletion test** before retaining it: delete or inline the layer when complexity disappears; keep it when deletion merely redistributes proven multi-caller, independent-change, or safety complexity. Treat compatibility as an explicit product contract, not a universal fallback. Code with no declared compatibility promise may remove a proven-dead path; **public APIs, persisted data, upgrade paths, and production migrations** require an approved transition, verification, and rollback plan. Never infer “no compatibility” from a pre-1.0 version alone when real users or data exist.
...
A solo developer may perform both passes at different times with fresh context. Require a second human only when project policy or the real production risk calls for one. Record ordinary review facts directly in structured evidence; create an external report only for independently inspectable findings. Treat **review feedback** as a hypothesis: verify it against the requirement, actual code use, compatibility constraints, tests, and YAGNI. Explain and reject advice that is wrong, excessive, or unused rather than implementing it because a reviewer proposed it.

Validate the real product, not only the build. Before assigning manual acceptance, confirm the real route/menu/role/account/device entry or carry an explicit `N/A` plus equivalent runtime/API evidence into the final result. If a required device or entry is `N/A`, name the unavailable path, the replacement method actually used, and the remaining gap; never hand that path to the user or operations. After two identical login/token/browser/channel failures, state the leading hypothesis as unconfirmed, preserve a safe state, and execute a different available method instead of repeating or delegating the same path:
