...
At every L1/L2/Emergency close, run the retrospective without asking the user to remember metrics. Runner drift, occupied tasks, resource conflicts, missing operation plans, and gate failures are deduplicated as task practice events. Pass `none` when no human exception exists. Only a judged workflow rework, unreasonable block/next action, bypass, or `hurt` impact becomes an evolution candidate; correct blocks remain statistics.
