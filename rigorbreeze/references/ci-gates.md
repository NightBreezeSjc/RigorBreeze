...
An `acceptance` check may opt into Verification Report v1. CI then validates the current Git SHA, minimum verification level, tracked Feature Map digest, mapped feature IDs, non-empty repository-relative evidence, and passed Doctor/Cleanup. The report and generated artifacts stay ignored or become time-limited CI artifacts; only their digests enter existing task evidence. Without this opt-in, CI and local behavior are unchanged. A passing report strengthens acceptance but never grants merge or production authority by itself.

Migration adapters must keep **from-schema preflight**, **migrate once**, and **to-schema assertions** distinct. The first phase runs on the deployed schema and guards every pending object reference with explicit detection. A successful migration is an immutable completed step: candidate or postcheck failure resumes after it rather than rerunning it. Existing migration rehearsal, backup/restore or forward-fix, operation-plan, and production-approval gates remain authoritative.
