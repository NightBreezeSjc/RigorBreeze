...
> **Public Preview：** v0.10.4 当前已经可以使用，并补齐真实交付暴露的项目塑形、工作流基线、过期或孤儿任务记录、可见演进提醒、未批准交付绕过、归档后交付、不完整或复合提示词、重复外部操作、压力下 Agent 行为和可避免的本地验证摩擦；但尚未完成 v1.0 所要求的验证，接口仍可能根据后续真实交付证据调整。
...
初始化并配置项目检查后，应在创建新的 L1/L2 任务前由人建立 Git 工作流基线；L0 继续保持轻量。`status --json` 会在 `workflowBaseline` 中报告真正基准分支的状态。用户明确授权后，Codex 可以执行 `automate commit --once --workflow-baseline --expected-head <SHA>`；它只暂存受管工作流文件，遇到混入产品改动或秘密时阻断，且不会持久化 Git 权限。安装后的 Skill 始终使用自身 v0.10.4 bundled runner 检查项目，分别报告缺失或被修改的组件，活动实施任务存在期间不会静默覆盖。活动合同缺失时，status 不再崩溃，而是报告 `orphaned-record` 和恢复动作；有意预建的草稿会展示精简的 `Task-Origin` 与 `Waiting-On`。已有演进候选会在状态中提示可复制的 `$rigorbreeze 汇总这个项目的演进候选`。高风险任务不能用手工任务卡替代损坏的工作流状态，必须先恢复记录或建立明确的 Emergency 合同。
...
v0.10.4 保持最小 Spec Tree 和现有命令面，同时让过期工作流恢复与已记录经验在下一次高风险写入前真正可见。已完成证据继续保留最终结果、最近一次历史失败、汇总次数、全部 profile 级验证历史、TDD、验收、实践和关闭事实；不会删除回归测试或非完成任务历史。更高成熟度仍必须来自反复真实使用，而不是继续增加功能。
