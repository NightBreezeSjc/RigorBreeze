...
> **Public Preview：** v0.13.0 当前已经可以使用。它在保留生产控制和普通个人开发轻量体验的同时，将可复用流程修复与业务任务隔离、复用顺序项目 worktree、把迁移检查限定为当前变化，并让源码证明保持精简且可由独立预期核验。它尚未完成 v1.0 所要求的验证，接口仍可能根据后续真实交付证据调整。
...
v0.13.0 保持最小 Spec Tree 和现有命令面，并吸收一次真实遗留源码基线交付的经验：历史破坏性 SQL 不再阻塞无关验证，当前任务修改的破坏性迁移仍会阻断；顺序项目复用 integration worktree；扫描器/证据工具按真实后果选择通道；可复用流程缺陷切到独立 Skill 任务；源码完整性必须由独立归属预期证明。已完成 evidence 继续精简，原始清单与大型生成快照默认保留在忽略目录或 CI Artifact，除非它本身就是批准交付物。
