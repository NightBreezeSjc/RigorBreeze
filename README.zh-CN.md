...
> **Public Preview：** v0.15.1 当前可以使用。它完整保留 v0.15 的可靠性门禁，同时通过更短的 Skill 入口、更小的项目常驻策略和精简项目状态投影减少日常上下文消耗。活动项目仍冻结其已安装 Runner，任务关闭后再升级。
...
初始化并配置检查后，应在创建 L1/L2 前建立 Git 工作流基线；Direct 和 L0 保持轻量。`status --json` 报告真实基准分支状态，明确授权后 Codex 可建立不混入产品代码的一次性基线提交。安装后的 Skill 使用自身 v0.15.1 bundled runner，分别报告缺失或被修改的组件，实施中不会静默升级。高风险合同丢失时必须恢复记录或建立明确 Emergency 合同，不能用手工任务卡绕过。
...
v0.15.1 完整保留 v0.15.0 的风险、TDD、验收和交付门禁，只调整上下文路由：激活后的 Skill 入口更短，默认读取当前任务状态，并行协调可请求向后兼容的 compact 投影，完整状态响应保持不变。公共 CLI 命令数量和第三方依赖不变化；状态/配置继续为 schema v5，完整 evidence 继续为 schema v4。
