...
> **Public Preview：** v0.14.0 当前可以使用。它新增风险自适应记录、无任务 Direct 通道、干净 L1 自动收口、安全本地清理和三行状态展示，同时不降低 L2、迁移、支付、权限和发布门禁。它仍需完成 v1.0 要求的真实交付验证。
...
初始化并配置检查后，应在创建 L1/L2 前建立 Git 工作流基线；Direct 和 L0 保持轻量。`status --json` 报告真实基准分支状态，明确授权后 Codex 可建立不混入产品代码的一次性基线提交。安装后的 Skill 使用自身 v0.14.0 bundled runner，分别报告缺失或被修改的组件，实施中不会静默升级。高风险合同丢失时必须恢复记录或建立明确 Emergency 合同，不能用手工任务卡绕过。
...
v0.14.0 在低后果场景删除日常仪式，在失败代价高的场景保留严谨性：小改走 Direct，L1 使用私有精简记录，L2/Emergency 使用完整私有证据和脱敏审计摘要，并自动完成干净任务收口、安全清理和三行状态展示。公共 CLI 命令数与第三方依赖不增加；状态/配置升级到 schema v5，完整 evidence 继续为 schema v4。
