...
> **Public Preview：** v0.16.0 当前可以使用。它完整保留 v0.15 的可靠性门禁，同时通过有序复用阶梯、跨仓单次交互、有界 evidence 和维护者 Token 遥测减少方案与流程浪费。活动项目仍冻结其已安装 Runner，任务关闭后再升级。
...
批准前，RigorBreeze 会检查占位符、内部矛盾、过大切片，以及结果/事实源/兜底语义歧义。复合需求会拆成可观察的 ADD/REMOVE/MOVE/RETAIN/REPLACE 原子，每项映射到验收或明确不做；扩大范围前先证明问题属于错误/缺失数据、错误配置还是缺少能力，并把最小修正与可选预防分开。UI 最终状态清单覆盖必须存在、必须不存在、位置/顺序和保留行为。否定表达会根据证据判断是在描述当前问题还是目标结果，只有方向仍会改变结果时才问一个简短问题。未被运行证据区分的根因只能标为假设；人工验收必须说明真实 route/menu/role/account/device 入口，或者诚实写成 `N/A` 并给出等价运行/API 证据。登录、token、浏览器或渠道连续两次同样失败后，Skill 会保留安全状态并切换方法，不再沿原路径重试。涉及三个或更多相关页面或新的视觉语言时，必须先做一个 visual tracer，除非只是精确复用已批准组件。完成声明必须说明本轮新运行的命令、退出状态和覆盖范围；评审建议先与仓库事实和 YAGNI 对照，推测性层级接受删除测试；同一缺陷连续三个假设失败后进入架构停点。维护者使用十七个确定性 Agent 压力场景验证这些规则；测试资产不会安装到业务项目，CI 也不会调用真实模型。
...
初始化并配置检查后，应在创建 L1/L2 前建立 Git 工作流基线；Direct 和 L0 保持轻量。`status --json` 报告真实基准分支状态，明确授权后 Codex 可建立不混入产品代码的一次性基线提交。安装后的 Skill 使用自身 v0.16.0 bundled runner，分别报告缺失或被修改的组件，实施中不会静默升级。高风险合同丢失时必须恢复记录或建立明确 Emergency 合同，不能用手工任务卡绕过。
...
日常开发只读取当前 worktree 的 `status --json`；在仓库和外部状态都没有变化的一段连续写入中可以复用该快照。并行协调使用 `status --all --compact --json`：保留活动任务、阻断、依赖、worktree 所有权和确定性下一动作，把历史 cleanup 明细压缩为计数。成功检查只汇总命令、退出码、覆盖范围和报告摘要，只有失败诊断才读取受限的原始输出。原有 `status --all --json` 完全保留，用于精确修复、清理和演进审计，因此这次上下文优化不会削弱机器门禁或删除证据。
...
v0.16.0 完整保留 v0.15 的风险、TDD、验收和交付门禁，增加有序最小方案阶梯、跨仓一个结果只进行一次人工交互但保留各仓证明、在线 evidence 压缩，以及用于诚实 A/B 的 Git 私有 Token 遥测。它不安装 Ponytail，也不增加模式、Hook、债务注释、公共命令、依赖或跨仓状态。状态/配置继续为 schema v5，完整 evidence 继续为 schema v4。
