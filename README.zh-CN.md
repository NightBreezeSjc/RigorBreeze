...
> **Public Preview：** v0.22.0 当前可以使用。它让checkpoint提交继续走正常完成生命周期，使compact协调状态真正精简，并只按相关章节加载高级规则。活动项目仍冻结其已安装 Runner，任务关闭后再升级。
...
初始化并配置检查后，应在创建 L1/L2 前建立 Git 工作流基线；Direct 和 L0 保持轻量。`status --json` 报告真实基准分支状态，明确授权后 Codex 可建立不混入产品代码的一次性基线提交。安装后的 Skill 使用自身 v0.22.0 bundled runner，分别报告缺失或被修改的组件，实施中不会静默升级。高风险合同丢失时必须恢复记录或建立明确 Emergency 合同，不能用手工任务卡绕过。
...
v0.22.0保留全部命令和门禁，并区分同一integration流中的活动checkpoint与历史外部集成。compact协调状态以确定性摘要代替完整Scope并投影一个安全收口动作；完整状态保持不变。高级Reference只按相关章节读取一次，不再默认整套加载。RigorBreeze仍是质量控制器，不是数据库解析器、浏览器、模拟器或调度平台。状态/配置继续为schema v5，完整evidence继续为schema v4，Automation Journal继续为v1。
