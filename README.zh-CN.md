...
> **Public Preview：** v0.9.0 当前已经可以使用，并补齐真实交付暴露的工作流基线、已集成旧任务、归档后交付、不完整提示词、重复外部操作和压力下 Agent 行为缺口；但尚未完成 v1.0 所要求的验证，接口仍可能根据后续真实交付证据调整。
...
批准前，RigorBreeze 会检查占位符、内部矛盾、过大切片，以及结果/事实源/兜底语义歧义。完成声明必须说明本轮新运行的命令、退出状态和覆盖范围；评审建议先与仓库事实和 YAGNI 对照；同一缺陷连续三个假设失败后进入架构停点，不继续堆第四个补丁。维护者使用六个合成 Agent 压力场景验证这些规则；测试资产不会安装到业务项目，CI 也不会调用真实模型。
...
初始化并配置项目检查后，应在首个 enforced L1/L2 批准前由人建立 Git 工作流基线。`status --json` 会在 `workflowBaseline` 中报告真正基准分支的状态。用户明确授权后，Codex 可以执行 `automate commit --once --workflow-baseline --expected-head <SHA>`；它只暂存受管工作流文件，遇到混入产品改动或秘密时阻断，且不会持久化 Git 权限。安装后的 Skill 始终使用自身 v0.9.0 bundled runner 检查项目，分别报告缺失或被修改的组件，活动实施任务存在期间不会静默覆盖。
...
v0.9.0 保持最小 Spec Tree 和现有命令面，把四条执行规则变成可压力验证的行为合同：批准前语义自检、完成前新鲜证据、评审意见核验，以及三个失败假设后的架构停点。历史 reconciled 只记录真实缺口，已关闭任务也不再暴露过期动作。更高成熟度仍必须来自反复真实使用，而不是继续增加功能。
