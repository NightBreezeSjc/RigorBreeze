...
> **Public Preview：** v0.18.0 当前可以使用。它保留全部生产门禁，同时要求 L1/L2 在干净工作树上批准，区分功能/PR与可独立验证任务切片，并让 UAT 或后续反馈后的写入重新经过当前状态和范围。活动项目仍冻结其已安装 Runner，任务关闭后再升级。
...
初始化并配置检查后，应在创建 L1/L2 前建立 Git 工作流基线；Direct 和 L0 保持轻量。`status --json` 报告真实基准分支状态，明确授权后 Codex 可建立不混入产品代码的一次性基线提交。安装后的 Skill 使用自身 v0.18.0 bundled runner，分别报告缺失或被修改的组件，实施中不会静默升级。高风险合同丢失时必须恢复记录或建立明确 Emergency 合同，不能用手工任务卡绕过。
...
v0.18.0 完整保留 v0.17 的风险、TDD、验收和交付门禁，在基线建立前阻断脏 L1/L2 批准，区分外来工作、缓存治理和新增范围漂移，区分可独立验证任务与最终功能PR，并要求验收/后续反馈后的写入重新进入当前状态与范围。Direct、L0保持轻量，Emergency不变。状态/配置继续为 schema v5，完整 evidence 继续为 schema v4。
