...
> **Public Preview：** v0.12.0 当前已经可以使用。它在保留真实交付生产控制和普通个人开发轻量体验的同时，增加受控决策前沿、单问题原型和抽象删除测试。它尚未完成 v1.0 所要求的验证，接口仍可能根据后续真实交付证据调整。
...
初始化并配置项目检查后，应在创建新的 L1/L2 任务前由人建立 Git 工作流基线；L0 继续保持轻量。`status --json` 会在 `workflowBaseline` 中报告真正基准分支的状态。用户明确授权后，Codex 可以执行 `automate commit --once --workflow-baseline --expected-head <SHA>`；它只暂存受管工作流文件，遇到混入产品改动或秘密时阻断，且不会持久化 Git 权限。安装后的 Skill 始终使用自身 v0.12.0 bundled runner 检查项目，分别报告缺失或被修改的组件，活动实施任务存在期间不会静默覆盖。活动合同缺失时，status 不再崩溃，而是报告 `orphaned-record` 和恢复动作；有意预建的草稿会展示精简的 `Task-Origin` 与 `Waiting-On`。已有演进候选会在状态中提示可复制的 `$rigorbreeze 汇总这个项目的演进候选`。高风险任务不能用手工任务卡替代损坏的工作流状态，必须先恢复记录或建立明确的 Emergency 合同。
...
v0.12.0 保持最小 Spec Tree 和现有命令面，同时改善 Agent 写代码前的决策质量：决策前沿限制提问，原型只回答一个问题，推测性抽象接受删除测试。已完成证据保留每个验收 ID 的最终有效 GREEN、最近一次有价值的历史失败、汇总次数、profile 级验证、验收、实践和关闭事实；不会压缩回归测试，也不会压缩活动、abandoned 或 reconciled 任务历史。仓库源码继续保存维护者回归测试，安装 ZIP 则排除维护者测试与缓存。任务合同和精简 evidence 继续作为版本化审计记录，原始日志和生成报告留在忽略目录或 CI 制品中。除非真实使用证明迁移和权威成本值得，否则不建立第二套本地 evidence 仓库。
