...
> **Public Preview：** v0.13.0 当前已经可以使用。它在保留生产控制和普通个人开发轻量体验的同时，将可复用流程修复与业务任务隔离、复用顺序项目 worktree，并把迁移检查限定为当前变化。它尚未完成 v1.0 所要求的验证，接口仍可能根据后续真实交付证据调整。
...
初始化并配置项目检查后，应在创建新的 L1/L2 任务前由人建立 Git 工作流基线；L0 继续保持轻量。`status --json` 会在 `workflowBaseline` 中报告真正基准分支的状态。用户明确授权后，Codex 可以执行 `automate commit --once --workflow-baseline --expected-head <SHA>`；它只暂存受管工作流文件，遇到混入产品改动或秘密时阻断，且不会持久化 Git 权限。安装后的 Skill 始终使用自身 v0.13.0 bundled runner 检查项目，分别报告缺失或被修改的组件，活动实施任务存在期间不会静默覆盖。活动合同缺失时，status 不再崩溃，而是报告 `orphaned-record` 和恢复动作；有意预建的草稿会展示精简的 `Task-Origin` 与 `Waiting-On`。已有演进候选会在状态中提示可复制的 `$rigorbreeze 汇总这个项目的演进候选`。高风险任务不能用手工任务卡替代损坏的工作流状态，必须先恢复记录或建立明确的 Emergency 合同。
...
RigorBreeze 源码仓库中的维护者任务合同、evidence 和 archive 只保存在本地，不再公开包含本机路径或临时命令输出的机器记录。公开发布历史由 Git 提交、变更记录、回归测试和 CI 承担；使用 RigorBreeze 初始化的业务项目仍遵循原有项目证据策略。
...
v0.13.0 保持最小 Spec Tree 和现有命令面，只吸收三项可复用的真实交付经验：历史破坏性 SQL 不再阻塞无关验证，当前任务修改的破坏性迁移仍会阻断；顺序项目复用 integration worktree；可复用流程缺陷切到独立 Skill 任务，不扩大业务任务范围。
