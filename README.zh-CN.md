...
> **Public Preview：** v0.19.0 当前可以使用。它保持v0.18的公共工作流不变，同时把记录生命周期、验证执行和诊断从CLI入口中分离。活动项目仍冻结其已安装Runner，任务关闭后再升级。
...
初始化并配置检查后，应在创建 L1/L2 前建立 Git 工作流基线；Direct 和 L0 保持轻量。`status --json` 报告真实基准分支状态，明确授权后 Codex 可建立不混入产品代码的一次性基线提交。安装后的 Skill 使用自身 v0.19.0 bundled runner，分别报告缺失或被修改的组件，实施中不会静默升级。高风险合同丢失时必须恢复记录或建立明确 Emergency 合同，不能用手工任务卡绕过。
...
v0.19.0 完整保留v0.18的命令、风险通道、TDD、验收和交付门禁，只将CLI入口收敛为编排层。记录、验证和诊断现在具有明确的无环模块边界和一份安装helper清单。状态/配置继续为schema v5，完整evidence继续为schema v4，Automation Journal继续为v1。
