...
> **Public Preview：** v0.17.0 当前可以使用。它保留全部生产门禁，同时让确定性修正走 Direct、将写者查询限制在目标路径、在 RED/构建前检查环境，并复用未变化的验证。活动项目仍冻结其已安装 Runner，任务关闭后再升级。
...
初始化并配置检查后，应在创建 L1/L2 前建立 Git 工作流基线；Direct 和 L0 保持轻量。`status --json` 报告真实基准分支状态，明确授权后 Codex 可建立不混入产品代码的一次性基线提交。安装后的 Skill 使用自身 v0.17.0 bundled runner，分别报告缺失或被修改的组件，实施中不会静默升级。高风险合同丢失时必须恢复记录或建立明确 Emergency 合同，不能用手工任务卡绕过。
...
v0.17.0 完整保留 v0.16 的风险、TDD、验收和交付门禁，增加路径级写者查询、stale registry 容错、RED/构建环境预检、pending 只读权威观测、临时合成 RSA 构建适配和未变化验证复用。Direct 不创建 task/evidence，也不能替代涉及 API、数据、认证、权限、支付、门锁、迁移、依赖、生产配置或外部状态的 L2。状态/配置继续为 schema v5，完整 evidence 继续为 schema v4。
